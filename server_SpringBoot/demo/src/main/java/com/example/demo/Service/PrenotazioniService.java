...
public class PrenotazioniService {
	...
	public PrenotazioniViewModel save(PrenotazioniViewModel pvm1) {
		...
		try {
			...
			/*
			 * QUESTO IF VERIFICA SE L'ORARIO E LA DATA DI PRENOTAZIONI SONO ACCETTABILI PER
			 * QUEL PRENOTABILE
			 */
			if ((VerificaData(DataIprenotabile, DataFprenotabile, DataPrenotaz)
				...
				/*
				 * QUESTO IF VERIFICA CHE IL PRENOTABILE SCELTO NON SIA GIA' STATO PRENOTATO DA
				 * UN ALTRO UTENTE IN QUELL'INTERVALLO DI TEMPO DI UN GIORNO SPECIFICO
				 */

				List<Prenotazioni> P = prenotazioniRepository.ControllaPrenotazione(p.getId_p(), DataPrenotaz,
						p.getPrenotabile().getCodice(), OraIPrenotaz, OraFPrenotaz);
	...
	public List<PrenotazioniViewModel> LoadAllPrenotazioni() {
		List<Prenotazioni> PList = prenotazioniRepository.findAll();
		List<PrenotazioniViewModel> PListVm = new ArrayList<PrenotazioniViewModel>();
		...
		if (PList.isEmpty()) {
			...
			for (Prenotazioni p : PList) {
				PListVm.add(prenotazioneConverter.toViewModel(p));
			}
			return PListVm;
		}
	}
	...
	public PrenotazioniViewModel deletePrenotazione(PrenotazioniViewModel pvm) {
		try {
			...
			Prenotazioni P = prenotazioniRepository.PrenotazioneUtente(pvm.getUtente(),
					prenotazioneDaEliminare.getId_p());
	...
	public PrenotazioniViewModel updatePrenotazione(PrenotazioniViewModel PvM) {
		...
		try {
			...
			Prenotazioni PrenotazionePresente = prenotazioniRepository.PrenotazioneUtente(PvM.getUtente(),
					PvM.getId_p());

			// IN BASE AI CAMPI CHE SONO STATI INSERITI NELLA PRENOTAZIONE DA MODIFICARE..SI
			// PROVVEDE A MODIFICARE QUELLO GIA' PRESENTE
			// NEL DB
			...
			/*
			 * PER CAPIRE QUALI CAMPI DEL PRENOTABILE VOGLIO AGGIORNARE,VEDO QUALI,FRA
			 * QUELLI DEL PrenotabileDaModificare NON SONO NULL E PROVVEDO AD USARE I LORO
			 * VALORI PER MODIFICARE L'EQUIVALENTE PRENOTABILE NEL DB
			 */

			if (!PrenotazioneleDaModificare.getData_prenotazione()
					.equals(PrenotazionePresente.getData_prenotazione())) {
				...
				List<Prenotazioni> P = prenotazioniRepository.ControllaPrenotazione(ID_P, dataPMod, CodicePreno,
						OraInizioP, OraFineP);
			...
			if (!PrenotazioneleDaModificare.getOra_inizio().equals(PrenotazionePresente.getOra_inizio())) {
				...
				List<Prenotazioni> P = prenotazioniRepository.ControllaPrenotazione(ID_P,
						PrenotazionePresente.getData_prenotazione(), CodicePreno, OraInizioPMod, OraFineP);
			...
			if (!PrenotazioneleDaModificare.getOra_fine().equals(PrenotazionePresente.getOra_fine())) {
				...
				List<Prenotazioni> P = prenotazioniRepository.ControllaPrenotazione(ID_P,
						PrenotazionePresente.getData_prenotazione(), CodicePreno, OraInizioP, OraFinePMod);
	...
	// ############################################################
	// VERIFICO CHE LA DATA DI PRENOTAZIONI RIENTRI NELL'INTERVALLO DI
	// DISPONIBILITA' DEL PRENOTABILE
	boolean VerificaData(Date DataPreI, Date DataPreF, Date DataPreNuova) {

		if (DataPreNuova.before(DataPreF) && DataPreNuova.after(DataPreI) && DataPreNuova.after(yesterday)
				|| DataPreNuova.equals(DataPreF)) {
			return true;
		} else
			return false;
	}

	// VERIFICO CHE L'ORA INIZIO DELLA PRENOTAZIONI RIENTRI NELL'INTERVALLO DI
	// DISPONIBILITA' DEL PRENOTABILE
	...
	// VERIFICO CHE L'ORA FINE DELLA PRENOTAZIONI RIENTRI NELL'INTERVALLO DI
	// DISPONIBILITA' DEL PRENOTABILE
